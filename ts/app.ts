...
import {CSanPham,CChicken} from './commom.js';
...
//SanPham
const code_mot_sp = (sp: TSanPham): string => {
    return `
    <div class="sp">
...
export const lay_sp_moi = async (so_sp) => {
    let sp_arr: TSanPham[];
    // limit là lấy slg vidu: limit(12) thì lấy 12 dòng
...
const code_mot_bv = (bv: IBaiViet): string => {
    return `
    <div class="bv">
        <h3>${bv.tieu_de}</h3>
        <p>${bv.mo_ta}</p>
...
const code_mot_bl = (bl: IBinhLuan): string => {
    return `
    <div class="bl">
        <h3>${bl.ho_ten}
...
export const laysptheoloai = async (id_loai: number, sosp:number=9) => {
    // let data: TLoai[];
    // limit là lấy slg vidu: limit(12) thì lấy 12 dòng
